...
Оговорки, без которых цифры выше врут:
  • Эффекты Чар в ядре работают, но агент пользуется только пассивными:
    обмены, цели, активации и предзаявки он не подаёт. Поэтому частота
    покупок мало говорит о балансе карт. Для чистого замера очков
    запускайте с --buy-rate 0.
  • Винрейт по Архимагам не считается: агенту Архимаг не раздаётся
    (archmage = null), все играют без персонажа.
